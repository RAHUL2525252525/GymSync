...
export default function Login() {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || "Could not reset password for this email.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div style={s.heroPanel}>
        ...
        <div style={s.heroBody}>
          ...
          <p style={s.heroTagline}>
            One place for check-ins, memberships and payments — built for people who don't skip leg day.
          </p>
        </div>

        {!isMobile && (
          <div style={s.pulseWrap}>
            <svg viewBox="0 0 300 60" style={s.pulseSvg} preserveAspectRatio="none">
              <path
        ...
        <div style={s.formInner}>
          <div style={s.formHeadRow}>
            ...
            <p style={s.formSubtitle}>
              ...
              {mode === "register" && "Join the floor. It takes less than a minute."}
              {mode === "forgot" && "We'll issue a temporary password for this email."}
            </p>
          </div>
          ...
          {mode !== "forgot" && (
            <div style={s.segmentSwitcher}>
              ...
              <button
                type="button"
                className="gs-segment-btn"
                style={{ ...s.segmentBtn, ...(mode === "register" ? s.segmentBtnActive : {}) }}
          ...
          {error && (
            <div
              style={{
                ...s.banner,
                ...(error.toLowerCase().includes("successful") ? s.bannerSuccess : s.bannerError),
              }}
            >
              {error}
            </div>
          ...
          {tempPassword && (
            <div style={s.tempPasswordBox}>
              <p style={s.tempPasswordLabel}>Your temporary password</p>
              <p style={s.tempPasswordValue}>{tempPassword}</p>
              <p style={s.tempPasswordHint}>Log in with this, then update your password.</p>
            </div>
          )}
          ...
          {mode === "forgot" ? (
            <form onSubmit={handleForgotPassword} style={s.form}>
              <label style={s.fieldLabel}>
                ...
                <input
                  ...
                  required
                  onFocus={(e) => (e.target.style.borderBottomColor = "#ff7a1a")}
                  onBlur={(e) => (e.target.style.borderBottomColor = "#e4ddd2")}
                />
              </label>
              <button
            ...
            <form onSubmit={mode === "register" ? handleRegister : handleLogin} style={s.form}>
              {mode === "register" && (
                <>
                  <label style={s.fieldLabel}>
                    Full name
          ...
          <p style={s.toggleText}>
            {mode === "forgot" && (
              <span className="gs-link-pill" style={s.toggleLink} onClick={() => switchMode("login")}>
                ← Back to login
              </span>
            )}
            {mode === "login" && (
              <>
                New member?{" "}
                <span className="gs-link-pill" style={s.toggleLink} onClick={() => switchMode("register")}>
                  Register here
                </span>
              </>
            )}
          ...
          <div style={s.hint}>
            <strong style={{ color: "#e2651a" }}>Admin login:</strong> use your admin email &amp; password configured in the backend .env
          </div>
        </div>
      </div>
    </div>
...
function EyeIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#ff7a1a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function getStyles(isMobile) {
  return {
    ...
    heroPanel: {
      ...
      background: "#0b0b0c",
      color: "#f5f0e8",
      padding: isMobile ? "24px 24px 28px" : "48px 56px",
      display: "flex",
      flexDirection: "column",
      justifyContent: isMobile ? "flex-start" : "space-between",
      clipPath: isMobile ? "none" : "polygon(0 0, 100% 0, 84% 100%, 0% 100%)",
    ...
    wordmark: {
      ...
      fontSize: isMobile ? "42px" : "clamp(48px, 6.5vw, 84px)",
